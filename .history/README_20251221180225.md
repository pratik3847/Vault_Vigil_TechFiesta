...
- [Detailed Feature Documentation](#-detailed-feature-documentation)
  ...
  - [Decision Engine](#5-decision-engine-decisionenginetsx)
...
### 4. **Decision Engine**
- Automated fraud decision-making with three actions:
...
The Guardian AI Dashboard follows a **4-stage fraud detection pipeline**:

```
┌─────────────────┐      ┌──────────────────┐      ┌─────────────────┐      ┌──────────────────┐
│   Transaction   │ ───▶ │  Fraud Link      │ ───▶ │  Risk Scoring   │ ───▶ │ Decision Engine  │
...
- **Process**: Generate or validate transaction data
- **Output**: Transaction record with initial metadata

### Stage 2: Fraud Link Analysis
- **Input**: Transaction with account IDs
- **Process**: 
  - Build network graph of connected accounts
  - Identify shared devices and IP addresses
  - Detect fraud chains and suspicious patterns
- **Output**: Network visualization and linked account data

### Stage 3: Risk Scoring
- **Input**: Transaction + Network analysis results
- **Process**: 
  - Evaluate multiple risk factors
  - Calculate weighted risk score (0-100)
  - Identify specific risk contributors
- **Output**: Comprehensive risk assessment report

### Stage 4: Decision Making
- **Input**: Risk score and analysis results
- **Process**: 
  - Apply decision rules based on risk thresholds
  - Generate fraud probability
  - Create detailed reasoning
- **Output**: Final decision (Allow/Alert/Block) with recommendations
...
- **D3.js Principles**: Physics-based layout algorithms
...
#### Features Section
Four feature cards explaining each module:
1. **Transaction Simulator**: Generate and test transaction patterns
2. **Fraud Link Analysis**: Visualize account networks
3. **Risk Scoring Engine**: Multi-factor risk assessment
4. **Decision Engine**: Automated decision-making
...
- **Account ID**: Selected account identifier
- **Account Type**: direct, shared_device, or shared_ip
- **Risk Level**: safe, warning, or fraud
- **Connection Type**: How accounts are linked
- **Transaction History**: Recent transactions involving this account

#### Fraud Chain Detection
Identifies suspicious patterns:
- **Fraud Chains**: Multiple fraud accounts linked together
- **Shared Resources**: Common devices/IPs across fraud accounts
- **Transaction Flow**: Money movement through the chain
- **Total Amount**: Cumulative value of suspicious transactions

**Example Fraud Chain**:
```
ACC-9012 (Fraud) --[DEV-C3D4]--> ACC-3456 (Fraud)
   └──[10.0.0.55]──┘
Total Transactions: 5 | Total Amount: $33,500
```
...
### 5. Decision Engine ([DecisionEngine.tsx](src/pages/DecisionEngine.tsx))

**Purpose**: Automated fraud decision-making with transparent reasoning.

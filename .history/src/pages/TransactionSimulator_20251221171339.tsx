 ...
 * 
 * Purpose: Generate and test both normal and fraudulent transaction patterns
 * 
 * Features:
 * - Manual transaction input with all required fields
 * - Automatic normal transaction generation (follows realistic patterns)
 * - Automatic fraud transaction generation (simulates anomalies)
 * - Real-time risk scoring based on multiple factors
 * 
 * MOCK AI LOGIC:
 * Fraud detection criteria:
 * - Amount > $10,000 (high-value transactions)
 * - Unknown or suspicious locations
 * - Repeated device/IP usage across multiple accounts
 * - Transaction velocity (rapid succession of transactions)
 * 
 * TODO: Replace with actual ML model API when backend is ready
 */
...
const TransactionSimulator = () => {
   ...
   * - Suspicious locations (contains "unknown", "lagos", "moscow")
   * - Missing location data
   */
  const detectFraud = (amount: number, location: string): boolean => {
    const suspiciousLocations = ["unknown", "lagos", "moscow", "rio"];
    const isHighValue = amount > 10000;
    const isSuspiciousLocation = suspiciousLocations.some(loc => 
      location.toLowerCase().includes(loc)
    );
    
    return isHighValue || isSuspiciousLocation || !location;
  };

  /**
   * RISK SCORE CALCULATION
   * Combines multiple fraud indicators into a 0-100 score
   * - Amount factor: Higher amounts = higher risk
   * - Location factor: Known fraud locations increase risk
   * - Pattern factor: Adds randomness to simulate ML uncertainty
   */
  const calculateRiskScore = (
    ...
    if (!isFraud) {
      return Math.floor(Math.random() * 30); // Low risk: 0-30
    }
    
    let riskScore = 50; // Base fraud risk
    
    // Amount factor
    if (amount > 10000) riskScore += 20;
    if (amount > 50000) riskScore += 10;
    ...
    // Add some variance to simulate ML model
  ...
  const submitCustomTransaction = () => {
    ...
    const location = formData.location || "Unknown";
    const isFraud = detectFraud(amount, location);
    const riskScore = calculateRiskScore(isFraud, amount, location);
